...
class ReportComplaintController extends Controller
{
    ...
    {
        $complaintstatus = ComplaintStatus::where('complaint_status.status', 'Y')
                ->where('complaint_status.is_delete', '0')
                ->join('complaint_status_types', 'complaint_status_types.id', 'complaint_status.complaint_status_type_id')
                ->select('complaint_status.*', 'complaint_status_types.type_name_en','complaint_status_types.type_name_si','complaint_status_types.type_name_ta')
                ->orderBy('complaint_status.status_en', 'ASC')
                ->orderBy('complaint_status_types.type_name_en', 'ASC')
                ->get();

        if(request()->ajax())
        {
            // $status = ComplaintStatus::where('complaint_status.status', 'Y')
            //     ->where('complaint_status.is_delete', '0')
            //     ->join('complaint_status_types', 'complaint_status_types.id', 'complaint_status.complaint_status_type_id')
            //     ->select('complaint_status.*', 'complaint_status_types.type_name_en','complaint_status_types.type_name_si','complaint_status_types.type_name_ta')
            //     ->orderBy('complaint_status.status_en', 'ASC')
            ...
            //             $totalcount = RegisterComplaint::whereDate('register_complaints.created_at', '>=', $from_date)->whereDate('register_complaints.created_at', '<=', $to_date)
            ...
            //                         ->select('complaint_status_id');
            //                         })
            //                         ->where('complaint_status_id','=', $item->id)
            //                         ->count();

            //             $closedcount = RegisterComplaint::where('register_complaints.complaint_status', 'Closed')
            //                         ->whereDate('register_complaints.created_at', '>=', $from_date)->whereDate('register_complaints.created_at', '<=', $to_date)
            //                         ->leftJoin('complaint_histories', function($query)
            //                         {
            //                         $query->on('register_complaints.id','=','complaint_histories.complaint_id')
            //                         ->whereRaw('complaint_histories.id IN (select MAX(a2.id) from complaint_histories as a2 join register_complaints as u2 on u2.id = a2.complaint_id group by u2.id)')
            ...
            //             $totalcount = RegisterComplaint::whereDate('register_complaints.created_at', '>=', $from_date)
            //                         ->leftJoin('complaint_histories', function($query)
            //                         {
            //                         $query->on('register_complaints.id','=','complaint_histories.complaint_id')
            //                         ->whereRaw('complaint_histories.id IN (select MAX(a2.id) from complaint_histories as a2 join register_complaints as u2 on u2.id = a2.complaint_id group by u2.id)')
            ...
            //             $closedcount = RegisterComplaint::where('register_complaints.complaint_status', 'Closed')
            //                         ->whereDate('register_complaints.created_at', '>=', $from_date)
            //                         ->leftJoin('complaint_histories', function($query)
            //                         {
            //                         $query->on('register_complaints.id','=','complaint_histories.complaint_id')
            //                         ->whereRaw('complaint_histories.id IN (select MAX(a2.id) from complaint_histories as a2 join register_complaints as u2 on u2.id = a2.complaint_id group by u2.id)')
            ...
            //         } else if($request->from_date == '' && $request->to_date != '') {
            //             $totalcount = RegisterComplaint::whereDate('register_complaints.created_at', '<=', $to_date)
            //                         ->leftJoin('complaint_histories', function($query)
            //                         {
            //                         $query->on('register_complaints.id','=','complaint_histories.complaint_id')
            //                         ->whereRaw('complaint_histories.id IN (select MAX(a2.id) from complaint_histories as a2 join register_complaints as u2 on u2.id = a2.complaint_id group by u2.id)')
            ...
            //             $closedcount = RegisterComplaint::where('register_complaints.complaint_status', 'Closed')
            //                         ->whereDate('register_complaints.created_at', '<=', $to_date)
            //                         ->leftJoin('complaint_histories', function($query)
            //                         {
            //                         $query->on('register_complaints.id','=','complaint_histories.complaint_id')
            //                         ->whereRaw('complaint_histories.id IN (select MAX(a2.id) from complaint_histories as a2 join register_complaints as u2 on u2.id = a2.complaint_id group by u2.id)')
            ...
            //             $totalcount = RegisterComplaint::leftJoin('complaint_histories', function($query)
            //                 {
            //                 $query->on('register_complaints.id','=','complaint_histories.complaint_id')
            //                 ->whereRaw('complaint_histories.id IN (select MAX(a2.id) from complaint_histories as a2 join register_complaints as u2 on u2.id = a2.complaint_id group by u2.id)')
            //                 ->select('complaint_status_id');
            ...
            //             $closedcount = RegisterComplaint::leftJoin('complaint_histories', function($query)
            //                 {
            //                 $query->on('register_complaints.id','=','complaint_histories.complaint_id')
            //                 ->whereRaw('complaint_histories.id IN (select MAX(a2.id) from complaint_histories as a2 join register_complaints as u2 on u2.id = a2.complaint_id group by u2.id)')
            //                 ->select('complaint_status_id');
            ...
            //             $status_name = $item->status_ta;
            //             $type_name = $item->type_name_ta;
            //         } else if (Session::get('applocale') == 'si'){
            //             $status_name = $item->status_si;
            //             $type_name = $item->type_name_si;
            //         } else {
            //             $status_name = $item->status_en;
            //             $type_name = $item->type_name_en;
            //         }

            //         $data[] = array(
            //             "act" => $status_name.' - '.$type_name,
            //             "totalcount" => $totalcount,
            //             "closedcount" => $closedcount,
            //             "pendingcount" => $pendingcount,
            //         );
            ...
            $offices = LabourOfficeDivision::where('status','Y')->where('is_delete', 0)->get();

            if(!empty($offices)){
                foreach($offices as $office){
                    $newcount= ComplaintHistory::where('sent_from_office', $office->id)->groupBy('complaint_id')->orderBy('created_at', 'DESC')->get();


                $data[] = array(
                    "act" => $office->office_name_en,
                    "totalcount" => $newcount
                );
                }
            }
