...
class PendingChargesheetListController extends Controller
{
    function __construct()
    {
        $this->middleware('permission:plaint-chargesheet-pending-list', ['only' => ['pendinglist']]);
        $this->middleware('permission:pending-chargesheet-list|pending-chargesheet-status-history|pending-chargesheet-action|pending-chargesheet-view', ['only' => ['list']]);
        $this->middleware('permission:pending-chargesheet-status-history', ['only' => ['complaintStatus']]);
        $this->middleware('permission:pending-chargesheet-action', ['only' => ['complaintAction', 'forward']]);
        $this->middleware('permission:pending-chargesheet-view', ['only' => ['view']]);
    }

    public function list(Request $request)
    {
        $office_id = Auth::user()->office_id;
        $approveCount = RegisterComplaint::where('complaint_status','Created - Plaint & Charge Sheet')
                            ->where('current_office_id',$office_id)
                            ->count();
        $rejectCount = RegisterComplaint::where('complaint_status','Rejected - Plaint & Charge Sheet')
                            ->where('current_office_id',$office_id)
                            ->count();
        $pendingCount = RegisterComplaint::where('action_type','Pending_plaint_charge_sheet')
                            ->where('current_office_id',$office_id)
                            ->count();
        ...
        if ($request->ajax()) {
            $data = RegisterComplaint::select('*')->where('action_type','Pending_plaint_charge_sheet')->where('current_office_id',$office_id);
        //dd($data); exit();
            return Datatables::of($data)
                ...
                ->addColumn('status', function ($row) {
                    $statusCount = ComplaintHistory::where('complaint_id', $row->id)
                                        ->count();
                    $edit_url = "";
                    $status_url = url('/pending-chargesheet-status-history/' . encrypt($row->id) . '');
                    $btn = '<a  href="'.$status_url.'" title="Frontoffice Remarks"><i class="fa fa-comments "></i></a> '.$statusCount;
                    return $btn;
                })
                ->addColumn('action', function ($row) {
                    $edit_url = "";
                    $status_url = url('/pending-chargesheet-action/' . encrypt($row->id) . '');
                ...
                //->addColumn('status', 'adminpanel.complaint.pendingChargesheetStatus')
                //->addColumn('action', 'adminpanel.complaint.pendingChargesheetAction')
                ->addColumn('view', function ($row) {
                    $view_url = url('/pending-chargesheet-view/' . encrypt($row->id) . '');
        ...
        return view('adminpanel.complaint.pendingChargesheetList', ['pendingCount' => $pendingCount, 'rejectCount' => $rejectCount, 'approveCount' => $approveCount, 'pendingApprovalCount' => $pendingApprovalCount ]);
    }
    ...
    public function complaintAction($id)
    {
        ...
        return view('adminpanel.complaint.action_chargesheet', ['data' => $data,'labourOffice' => $office, 'history' => $history, 'complaintstatus' => $complaintstatus, 'remarks' => $remarks, 'forwardTypes' => $ftype]);
    }
    ...
    public function forward(Request $request)
    {
        ...
        $insert['action_type'] = $request->action_type;
        $insert['remark'] = $request->remark;
        $insert['user_id'] = Auth::user()->id;
        $insert['forward_type_id'] = 0;
        $insert['complaint_status_id'] = 0;

        if($request->action_type == 'Create_plaint_and_charge_sheet'){
            if($complaint->pref_lang == "SI") {
                $insert['status_des'] = 'පැමිණිල්ල හා චෝදනා පත්‍රය සකස්කරන ලදී';
            } else if($complaint->pref_lang == "TA") {
                $insert['status_des'] = 'முறைப்பாட்டு மற்றும் குற்றப்பத்திரிகை தயாரிக்கப்பட்டுள்ளதோடு,  மேலதிக நடவடிக்கைகளுக்காக வரிசைப்படுத்தப்பட்டுள்ளது';
            } else {
                $insert['status_des'] = 'plaint & charge sheet created, Qued for the futher actions';
            }
            $forward = 'Plaint and Charge Sheet created';
        } else {
    ...
    public function approvedList(Request $request)
    {
        ...
        $rejectCount = RegisterComplaint::where('complaint_status','Rejected - Plaint & Charge Sheet')
                            ->where('current_office_id',$office_id)
                            ->count();
        $pendingCount = RegisterComplaint::where('complaint_status','Pending - Plaint & Charge Sheet')
                            ->where('current_office_id',$office_id)
                            ->count();

        if ($request->ajax()) {
            $data = RegisterComplaint::select('*')->where('complaint_status','Created - Plaint & Charge Sheet')->where('current_office_id',$office_id)->orderBy('id', 'DESC');
        //var_dump($data); exit();
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('status', function ($row) {
        ...
        return view('adminpanel.complaint.createdChargesheetList', ['approveCount' => $approveCount, 'rejectCount' => $rejectCount, 'pendingCount' => $pendingCount ]);
    }
    ...
    public function rejectedList(Request $request)
    {
        ...
        if ($request->ajax()) {
            $data = RegisterComplaint::select('*')->where('complaint_status','Rejected - Plaint & Charge Sheet')->where('current_office_id',$office_id)->orderBy('id', 'DESC');
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('status', function ($row) {
                    $statusCount = ComplaintHistory::where('complaint_id', $row->id)
        ...
        return view('adminpanel.complaint.rejectedChargesheetList', ['approveCount' => $approveCount, 'rejectCount' => $rejectCount, 'pendingCount' => $pendingCount ]);
    }
    ...
    public function statusUpdate(Request $request)
    {
        ...
        DB::commit();
        return redirect()->route('pending-chargesheet-list')
