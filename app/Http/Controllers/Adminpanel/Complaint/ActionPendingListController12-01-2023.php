...
class ActionPendingListController extends Controller
{
    ...
    public function complaintAction($id)
    {
        ...
        if($useroffice->office_type_id == 3){
            $loOfficers = User::role(['Labour Officer','DCL','ACL'])->where('office_id', Auth::user()->office_id)->where('is_delete', 0)->where('status', 'Y')->get();
        } else if ($useroffice->office_type_id == 4 || $useroffice->office_type_id == 5){
            $loOfficers = User::role(['Labour Officer','SLO','ACL'])->where('office_id', Auth::user()->office_id)->where('is_delete', 0)->where('status', 'Y')->get();
        } else {
