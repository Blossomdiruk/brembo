@section('title', 'Pending Certificate')
            ...
            <div class="col-lg-12">
                    <div class="row cms_top_btn_row" style="margin-left:auto;margin-right:auto;">
                        <a href="{{ route('pending-certificate-list') }}">
                            <button class="btn cms_top_btn top_btn_height cms_top_btn_active">{{ __('pendingcertificatelist.pending') }} <br><span class="txt-color-blue" onclick="" style=" text-align: center;display: contents;">{{ $pendingCount }}</span></button>
                        </a>
                    </div>
                </div>
                <!-- <div class="col-lg-8">
                    ...
                    <ul id="sparks" class="" style="position: relative; top: 10px;">
                            <li class="sparks-info sparks-info_active" style="border: 1px solid #c5c5c5; padding-right: 0px; padding: 10px; min-width: auto; max-width: auto;  transform: translate(0%, -12%);">
                                <a href="{{ route('pending-certificate-list') }}">
                                    <h5> {{ __('pendingcertificatelist.pending') }} <span class="txt-color-blue" onclick="" style=" text-align: center">{{ $pendingCount }}</span></h5>
                                </a>
                                    ...
                                    <table class="table table-bordered data-table" width="100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th width='15%' style="text-align: center; font-size: 11px">{{ __('pendingcertificatelist.ref_num') }}</th>
                                                <th width='15%' style="text-align: center; font-size: 11px">{{ __('pendingcertificatelist.external_ref_no') }}</th>
                                                <th width='15%' style="text-align: center; font-size: 11px">{{ __('pendingcertificatelist.complaint_name') }}</th>
                                                <th width='10%' style="text-align: center; font-size: 11px">{{ __('pendingcertificatelist.nic') }}</th>
                                                <th width='10%' style="text-align: center">{{ __('pendingcertificatelist.date') }}</th>
                                                <th width='2%' style="text-align:center;">{{ __('actionpendinglist.online_manual') }}</th>
                                                <th width='6%' style="text-align: center; font-size: 11px">{{ __('pendingcertificatelist.status') }} </th>
                                                <th width='6%' style="text-align: center; font-size: 11px">{{ __('pendingcertificatelist.action') }}</th>
                                                <th width='6%' style="text-align: center; font-size: 11px">{{ __('pendingcertificatelist.upload') }}</th>
                                                <th width='6%' style="text-align: center; font-size: 11px">{{ __('pendingcertificatelist.view') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                        </tbody>
        ...
        <script type="text/javascript">
            $(function() {
                ...
                var table = $('.data-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('pending-certificate-list') }}",
                    order: [ 0, 'desc' ],
                    columns: [{
                            data: 'DT_RowIndex',
                            name: 'id'
