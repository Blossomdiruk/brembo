            ...
            <div class="col-lg-12">
                    <div class="row cms_top_btn_row" style="margin-left:auto;margin-right:auto;">
                        ...
                        <a href="{{ route('plaint-chargesheet-pending-list') }}">
                            <button class="btn cms_top_btn top_btn_height cms_top_btn_active">{{ __('actionpendinglist.plaint') }} <br><span class="txt-color-blue" onclick="" style=" text-align: center;display: contents;">{{ $chargesheetCount }}</span></button>
                        </a>
                    ...
                    <ul id="sparks" class="">
                        <ul id="sparks" class="" style="position: relative; top: 10px;">
                            <li class="sparks-info" style="border: 1px solid #c5c5c5; padding-right: 0px; padding: 10px; min-width: auto; max-width: auto;  transform: translate(0%, -12%);">
                                <a href="{{ 'action-pending-list' }}">
                                    <h5> {{ __('pendingchargesheetlist.action_pending') }} <span class="txt-color-blue" onclick="" style=" text-align: center">{{ $pendingCount }}</span></h5>
                                </a>
                            </li>

                            <li class="sparks-info" style="border: 1px solid #c5c5c5; padding-right: 0px; padding: 10px; min-width: auto; max-width: auto; transform: translate(0%, -12%);">
                                <a href="{{ route('investigation-ongoing-list') }}">
                                    <h5> {{ __('pendingchargesheetlist.investigation_ongoing') }} <span class="txt-color-blue" onclick="" style=" text-align: center">{{ $ongoingCount }}</span></h5>
                                </a>
                            </li>
                            ...
                            <li class="sparks-info" style="border: 1px solid #c5c5c5; padding-right: 0px; padding: 27px 10px !important; min-width: auto;  max-width: auto; transform: translate(0%, -12%);">
                                <a href="{{ route('recovery-pending-list') }}">
                                    <h5> {{ __('pendingchargesheetlist.recovery_pending') }} <span class="txt-color-blue" style=" text-align: center"><i class=""></i>{{ $recoveryCount }}</span></h5>
                                </a>
                            </li>

                            <li class="sparks-info" style="border: 1px solid #c5c5c5; padding-right: 0px; padding: 10px; min-width: auto;  max-width: auto; transform: translate(0%, -12%);">
                                <a href="{{ route('legal-certificate-pending-list') }}">
                                    <h5> {{ __('pendingchargesheetlist.leagle') }}<span class="txt-color-blue" style=" text-align: center"><i class=""></i>{{ $certificateCount }}</span></h5>
                                </a>
                            </li>
                            ...
                            <li class="sparks-info sparks-info_active" style="border: 1px solid #c5c5c5; padding-right: 0px; padding: 10px; min-width: auto;  max-width: auto; transform: translate(0%, -12%);">
                                <a href="{{ route('plaint-chargesheet-pending-list') }}">
                                    <h5> {{ __('pendingchargesheetlist.plaint') }}<span class="txt-color-blue" style=" text-align: center"><i class=""></i>{{ $chargesheetCount }}</span></h5>
                                </a>
                            </li>

                            <li class="sparks-info" style="border: 1px solid #c5c5c5; padding-right: 0px; padding: 10px; min-width: auto;  max-width: auto; transform: translate(0%, -12%);">
                                <a href="{{ route('temporary-closed-list') }}">
                                    <h5> {{ __('pendingchargesheetlist.temp_close') }}<span class="txt-color-blue" style=" text-align: center"><i class=""></i>{{ $tempClosedCount }}</span></h5>
                                </a>
                            </li>

                            <li class="sparks-info" style="border: 1px solid #c5c5c5; padding-right: 0px; padding: 27px 10px !important; min-width: auto;  max-width: auto; transform: translate(0%, -12%);">
                                <a href="{{ route('closed-list') }}">
                                    <h5> {{ __('pendingchargesheetlist.close') }} <span class="txt-color-blue" style=" text-align: center"><i class=""></i>{{ $closedCount }}</span></h5>
                                </a>
                            </li>
                        </ul>
                    </ul>
                    ...
                    <article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        ...
                        <div class="jarviswidget jarviswidget-color-darken" id="user_types" data-widget-editbutton="false">
                            <header>
                                ...
                                <h2>{{ __('pendingchargesheetlist.title') }}</h2>
                            </header>
                            <!-- widget div-->
                            <div>
                                <!-- widget edit box -->
                                    ...
                                    <table class="table table-bordered data-table" width="100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th width='2%' style="text-align:center;">{{ __('pendingchargesheetlist.id') }}</th>
                                                <th width='21%' style="text-align: center; font-size: 11px">{{ __('pendingchargesheetlist.ref_num') }}</th>
                                                <th width='21%' style="text-align: center; font-size: 11px">{{ __('pendingchargesheetlist.external_ref_no') }}</th>
                                                <th style="text-align: center; font-size: 11px">{{ __('pendingchargesheetlist.complaint_name') }}</th>
                                                <th width='9%' style="text-align: center; font-size: 11px">{{ __('pendingchargesheetlist.nic') }}</th>
                                                <th width='9%' style="text-align: center; font-size: 11px">{{ __('pendingchargesheetlist.complainant_mobile') }}</th>
                                                <th width='4%' style="text-align: center">{{ __('pendingchargesheetlist.date') }}</th>
                                                <th width='2%' style="text-align:center;">{{ __('actionpendinglist.online_manual') }}</th>
                                                <th width='4%' style="text-align: center; font-size: 11px">{{ __('pendingchargesheetlist.status') }} </th>
                                                <!-- <th width='5%' style="text-align: center; font-size: 11px">{{ __('pendingchargesheetlist.action') }}</th> -->
                                                <th width='5%' style="text-align: center; font-size: 10px">{{ __('pendingchargesheetlist.upload') }}</th>
                                                <th width='4%' style="text-align: center; font-size: 11px">{{ __('pendingchargesheetlist.modify') }}</th>
                                                <th width='4%' style="text-align: center; font-size: 11px">{{ __('pendingchargesheetlist.view') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                        </tbody>
        ...
        <script type="text/javascript">
            $(function() {
                ...
                var table = $('.data-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('plaint-chargesheet-pending-list') }}",
                    order: [ 0, 'desc' ],
                    columns: [{
                            data: 'DT_RowIndex',
                            name: 'id'
